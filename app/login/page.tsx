...
export default function LoginPage() {
  ...
  const onGoogle = async () => {
    ...
    setLoading(false);
    if (error) { setError(t("login.googleError")); return; }
    ...
    <div className="max-w-sm mx-auto mt-20 p-6 border border-neutral-200 rounded-lg space-y-4">
      ...
      <form onSubmit={onSubmit} className="space-y-3">
        ...
        <div className="flex flex-col gap-2">
          <button type="submit" disabled={loading} className="w-full rounded-md border border-neutral-900 bg-neutral-900 text-white px-4 py-2">{loading ? t("login.signingIn") : t("login.signIn")}</button>
      ...
      <div className="text-xs text-neutral-600">{t("login.note")}</div>
    </div>
  );
}
